...
if ($formular_gesendet) {
    ...
    if (isset($_GET["studiengang"])) {
        $studiengang = trim($_GET["studiengang"]);
    }
    ...
    if (empty($name) || empty($studiengang) || empty($fach)) {
        $meldung = "Bitte füllen Sie alle Felder aus.";
    } elseif (strlen($name) < 2) {
        $meldung = "Der Name muss mindestens zwei Zeichen lang sein.";
    } elseif (strlen($studiengang) < 5) {
        $meldung = "Der Studiengang muss mindestens fünf Zeichen lang sein.";
    } else {
...
<body>
    <main>
        ...
        if ($formular_validiert) {
            ...
            echo "<p><strong>Studiengang:</strong> ";
            echo htmlspecialchars($studiengang);
            echo "</p>";
